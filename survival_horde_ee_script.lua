...
CreateWaves = function()
    WaveTable = import('/maps/survival_horde_ee.v0010/src/WaveTable.lua').getWaveTable(
        EnableLane1,
        EnableLane2,
        EnableLane3,
        EnableLane4,
...
CreateWaveTableOrders = function()
    ...
    WaveOrders = import('/maps/survival_horde_ee.v0010/src/WaveOrders.lua').getWaveTable(
        WaveDelay,
        ShuffleList,
...
--    local survival = import('/maps/survival_horde_ee.v0010/src/Survival.lua').newInstance(
--        ScenarioInfo,
--        CreateWaves,
...
ForkThread(function()
    local textPrinter = import('/maps/survival_horde_ee.v0010/src/lib/TextPrinter.lua').newInstance()
    ...
    textPrinter.print(string.rep( " ", 20 ) .. "Entropy Edition, version 10", textOptions)
    textPrinter.printBlankLine(textOptions)
    textPrinter.printBlankLine(textOptions)
...
local function newAirwingSpawner()
    ...
    return import('/maps/survival_horde_ee.v0010/src/lib/AirwingSpawner.lua').newInstance(mapSizeX, mapSizeY, "ARMY_SURVIVAL_ENEMY")
end

ForkThread(function()
    local textPrinter = import('/maps/survival_horde_ee.v0010/src/lib/TextPrinter.lua').newInstance()
    if ScenarioInfo.Options.opt_HordeAirWaves == 1 then
        local airwings = import('/maps/survival_horde_ee.v0010/src/Airwings.lua').newInstance(
            newAirwingSpawner(),
            textPrinter,
            TotalGameTime
        )
...
local function setupAutoReclaim()
    if ScenarioInfo.Options.opt_HordeAutoReclaim > 0 then
        ForkThread(
            import('/maps/survival_horde_ee.v0010/src/lib/AutoReclaim.lua').AutoResourceThread,
